  ...
  try {
    ...
    if (d.formType === 'referral') {
      ...
      if (refSheet.getLastRow() === 0) {
        refSheet.appendRow(['Timestamp','Referred By','Candidate Name','Candidate Phone','Notes','Status','Start Date','Payout 1','Payout 2']);
      }
      refSheet.appendRow([
        new Date().toLocaleString('en-CA'),
        d.yourName, d.theirName, d.theirPhone, d.notes, '', '', '', ''
      ]);

      var telHref = 'tel:' + String(d.theirPhone).replace(/[^0-9+]/g, '');
          ...
          '<div style="background:#fff;border-radius:4px;overflow:hidden">' +
            ...
            '<div style="padding:24px 32px">' +
              '<table style="width:100%;border-collapse:collapse">' +
                '<tr><td style="padding:10px 0;border-bottom:1px solid #eee"><span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#888">Candidate</span><br><span style="font-size:15px;color:#333">' + d.theirName + '</span></td></tr>' +
                '<tr><td style="padding:18px 0;border-bottom:1px solid #eee;text-align:center">' +
                  '<a href="' + telHref + '" style="display:inline-block;background:#D10000;color:#fff;font-size:22px;font-weight:700;letter-spacing:.5px;padding:16px 32px;border-radius:6px;text-decoration:none">📞 Call ' + d.theirPhone + '</a>' +
                '</td></tr>' +
                '<tr><td style="padding:10px 0;border-bottom:1px solid #eee"><span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#888">Referred By</span><br><span style="font-size:15px;color:#333">' + d.yourName + '</span></td></tr>' +
                (d.notes ? '<tr><td style="padding:10px 0;border-bottom:1px solid #eee"><span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#888">Notes</span><br><span style="font-size:15px;color:#333">' + d.notes + '</span></td></tr>' : '') +
              '</table>' +
            '</div>' +
            '<div style="background:#f5f5f5;padding:16px 32px;text-align:center;border-top:1px solid #eee">' +
              '<span style="font-size:12px;color:#999">Submitted via slinstallations.ca/refer | ' + new Date().toLocaleString('en-CA') + '</span>' +
            '</div>' +
          '</div>' +
        '</div>';
    ...
    if (d.formType === 'websiteUpdate') {
      ...
      UPDATE_EMAILS.forEach(function(email) {
        MailApp.sendEmail(email, '📝 SL Installations Website Update: ' + d.updateType, '', {htmlBody: devHtml, name: 'SL Update Form'});
      });
